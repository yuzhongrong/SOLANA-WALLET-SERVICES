...
import { solanaConnection } from "../../../SolanaConnection";
  ...
  export async function transactionSenderAndConfirmationWaiter({
    ...
    try {
      ...
      await Promise.race([
        mAlchemySolanaConnection.confirmTransaction(
          {
            ...blockhashWithExpiryBlockHeight,
        ...
        new Promise(async (resolve) => {
          ...
          while (!abortSignal.aborted) {
            await wait(2_000);
            const tx = await mAlchemySolanaConnection.getSignatureStatus(txid, {
    ...
    const response = promiseRetry(
      async (retry) => {
        const response = await mAlchemySolanaConnection.getTransaction(txid, {
